...
export class IntegrationsController {
  ...
  @Post('calendly')
  @ApiOperation({ summary: 'Enregistre ou met u00e0 jour une intu00e9gration Calendly' })
  ...
  async saveCalendlyIntegration(@Request() req, @Body() dto: SaveCalendlyDto) {
    this.logger.log(`Enregistrement de l'intu00e9gration Calendly pour l'utilisateur ${req.user.id}`);
    const success = await this.integrationsService.saveCalendlyIntegration(req.user.id, dto);
    return { success };
  }
